<?php
 ...
 if(isset($_POST['filtroAvanzato'])){
  ...
  foreach($result as $k => $val){
    if(!empty($val['jtei_art_year'])){
      $selected_year_raw[] = $val['jtei_art_year'];
      ...
      <div class="row">
              <div class="col-md-4 offset-1 " style="text-align:justify;margin-right:1%;" >
                  <h3><a href="https://journals.openedition.org/jtei/">JTEI</a></h3>
                      <p>Il Journal of the Text Encoding Initiative è la gazzetta ufficiale del Text Encoding Initiative Consortium. Pubblica gli atti della conferenza annuale TEI e dell'assemblea dei membri e questioni tematiche speciali: rapporti all'avanguardia 
                        sull'editing testuale elettronico, tendenze attuali nella codifica TEI e nuovi casi d'uso per TEI.
                        Fornisce inoltre un forum per articoli sulla discussione dell'interfaccia tra il TEI e altre comunità e, più in generale, sul ruolo degli standard tecnologici nelle digital humanities, tra cui l'editing accademico digitale, l'analisi linguistica, la creazione di corpora e aree più recenti come 
                              ...
                              <label>Anno</label>              
                                <select  name="anno[]" id="anno" class="form-control selectpicker" multiple>                                   
                                  <?php foreach($year_arr as $k => $val){?>
                                    <option value="<?= $val['jtei_art_year'] ?>"
                                    <?php 
                                    if(isset($_POST['filtroAvanzato'])){
                                      if( in_array($val['jtei_art_year'],$selected_year)){
                                        print 'selected';
                                      }
                                    }
                                  ?>
                                    ><?= $val['jtei_art_year'] ?></option>
                                  <?php }?>                              
                                </select>     
                              ...
                              <select  name="annoDA" id="annoDA" class="form-control selectpicker">  
                                <option value="">   </option>                      
                                  <?php foreach(array_reverse($year_arr) as $k => $val){?>
                                    ...
                                    <?php 
                                    if(isset($_POST['filtroAvanzato'])){
                                      if($val['jtei_art_year'] == $_POST['annoDA']){
                                        print 'selected';
                                      }
                                    }
                                    ?>
                              ...
                              <select  name="annoA" id="annoA" class="form-control selectpicker">  
                                <option value=""></option>                           
                                  <?php foreach($year_arr as $k => $val){?>
                                    ...
                                    <?php 
                                    if(isset($_POST['filtroAvanzato'])){
                                      if($val['jtei_art_year'] == $_POST['annoA']){
                                        print 'selected';
                                      }
                                    }
                                    ?>><?= $val['jtei_art_year'] ?></option>
        ...
        <div class="table-responsive-nomarginpadding">
              <table id="myDatatable" class="table table-striped" cellspacing="0" width="100%">
                ...
                <tr>
                    <td class="hidden-xs"><?= $value['jtei_art_id'] ?></td>
                    <td><?=  $value['jtei_art_author']  ?></td>
                    <td  class="focus"><a href="<?= $value['jtei_art_link'] ?>"> <?= $value['jtei_art_title'] ?></a></td>          
                    <td><?=  $value['jtei_art_year']  ?></td> 
                <?php }?>
                </tbody>
              </table>
          </div>
